...
// BKAV HaiHS: Component cửa sổ dùng chung để gộp nhanh hàng loạt nhân sự vào nhóm quyền chỉ định - start
const BulkAddToGroupModal = ({ isOpen, onClose, selectedUsers, onSuccess }) => {
  ...
  // BKAV HaiHS : Reset trạng thái tìm kiếm và nhóm được chọn khi modal được mở - start
  useEffect(() => {
    setSelectedGroup(null);
    setGroupSearchKeyword("");
    setIsDropdownOpen(false);
  }, [isOpen]);
  // BKAV HaiHS : Reset trạng thái tìm kiếm và nhóm được chọn khi modal được mở - end

  // BKAV HaiHS : Bộ lọc cho Dropdown tìm kiếm nhóm với debounce 500ms - start
  useEffect(() => {
    if (!isOpen) return;
  ...
  // BKAV HaiHS : Bộ lọc cho Dropdown tìm kiếm nhóm với debounce 500ms - end

  // BKAV HaiHS : Sử dụng useOutsideClick hook để tự động đóng dropdown tìm kiếm nhóm khi click ra ngoài - start
  useOutsideClick(dropdownRef, () => setIsDropdownOpen(false));
  // BKAV HaiHS : Sử dụng useOutsideClick hook để tự động đóng dropdown tìm kiếm nhóm khi click ra ngoài - end

  // BKAV HaiHS : Tải danh sách nhóm quyền từ API tìm kiếm hỗ trợ phân trang - start
  ...
  // BKAV HaiHS : Tải danh sách nhóm quyền từ API tìm kiếm hỗ trợ phân trang - end

  // BKAV HaiHS : Bật hoặc tắt danh sách dropdown chọn nhóm và nạp dữ liệu lần đầu nếu trống - start
  ...
  // BKAV HaiHS : Bật hoặc tắt danh sách dropdown chọn nhóm và nạp dữ liệu lần đầu nếu trống - end

  // BKAV HaiHS : Xử lý sự kiện cuộn danh sách dropdown để tải thêm dữ liệu nhóm - start
  ...
  // BKAV HaiHS : Xử lý sự kiện cuộn danh sách dropdown để tải thêm dữ liệu nhóm - end

  // BKAV HaiHS : Thực thi gửi yêu cầu thêm hàng loạt người dùng vào nhóm quyền đã chọn - start
  ...
  // BKAV HaiHS : Thực thi gửi yêu cầu thêm hàng loạt người dùng vào nhóm quyền đã chọn - end
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#161b26] border border-gray-200 dark:border-[#232d42] rounded-2xl shadow-2xl flex flex-col relative overflow-visible transition-colors duration-300">
        ...
        >
          ...
          >
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 right-0 mt-1.5 bg-white dark:bg-[#1a202c] border border-gray-200 dark:border-[#232d42] rounded-xl shadow-2xl z-[100] animate-fade-in transition-colors overflow-hidden flex flex-col max-h-[280px]">
                {/* ô tìm kiếm nhóm */}
...
// BKAV HaiHS: Component cửa sổ dùng chung để gộp nhanh hàng loạt nhân sự vào nhóm quyền chỉ định - end
